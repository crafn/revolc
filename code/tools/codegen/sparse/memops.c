...
static int find_dominating_parents_memops(pseudo_t pseudo, struct instruction *insn,
...
no_dominance:
		...
		if (!find_dominating_parents_memops(pseudo, insn, parent, generation, dominators, local, loads))
			return 0;
		continue;
...
static void simplify_loads(struct basic_block *bb)
{
	...
	FOR_EACH_PTR_REVERSE(bb->insns, insn) {
		...
		if (insn->opcode == OP_LOAD) {
			...
			dominators = NULL;
			if (find_dominating_parents_memops(pseudo, insn, bb, generation, &dominators, local, 1)) {
...
static void kill_store_memops(struct instruction *insn)
{
	if (insn) {
		insn->bb = NULL;
		insn->opcode = OP_SNOP;
...
static void kill_dominated_stores_memops(struct basic_block *bb)
{
	struct instruction *insn;
...
void simplify_memops(struct entrypoint *ep)
{
	...
	FOR_EACH_PTR_REVERSE(ep->bbs, bb) {
		kill_dominated_stores_memops(bb);
	} END_FOR_EACH_PTR_REVERSE(bb);
}
