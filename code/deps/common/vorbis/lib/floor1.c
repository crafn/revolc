...
static int ilog2_floor1(unsigned int v){
  int ret=0;
  if(v)--v;
  while(v){
    ret++;
...
static void floor1_pack (vorbis_info_floor *i,oggpack_buffer *opb){
  ...
  oggpack_write(opb,ilog2_floor1(maxposit),4);
  rangebits=ilog2_floor1(maxposit);
...
static const float FLOOR1_fromdB_LOOKUP_floor1[256]={
...
static void render_line(int n, int x0,int x1,int y0,int y1,float *d){
  ...
  if(x<n)
    d[x]*=FLOOR1_fromdB_LOOKUP_floor1[y];
...
static int floor1_inverse2(vorbis_block *vb,vorbis_look_floor *in,void *memo,
  ...
  if(memo){
    ...
    for(j=hx;j<n;j++)out[j]*=FLOOR1_fromdB_LOOKUP_floor1[ly]; /* be certain */
    return(1);
  }
